...
- `-seed N` — Base seed for procedural level generation (default `0`). For each level, generation uses `N`, then `N+1`, `N+2`, … (up to a fixed try limit) until a board is built, so a failing draw does not abort the game. The same `-seed` still yields the same sequence of levels.
